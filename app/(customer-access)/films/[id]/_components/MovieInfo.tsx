...
export function MovieInfo({ movie, ratings }: MovieInfoProps) {
  ...
  return (
    <motion.div>
      <motion.div
        className="grid grid-cols-1 md:grid-cols-2 p-10 items-center gap-10 relative"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <motion.div className="px-10 items-center gap-10 relative">
        <motion.div
          className="col-span-1 space-y-6 p-8 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 shadow-xl"
          {...fadeInUp}
        >
          ...
          <div className="flex justify-center">
            <Button onClick={() => setIsModalOpen(true)} className="w-[50%]">
              Réserver
            </Button>
          </div>
        </motion.div>
